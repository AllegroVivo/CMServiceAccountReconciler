...
__all__ = ("DuctCleaningWorksheet",)

################################################################################
class DuctCleaningWorksheet(_WorksheetBase):
    ...
    def _parse_record(self, row: List[Dict[str, Any]], row_index: int) -> Optional[SheetRecord]:
        ...
        csr_highlight = row[3]["effectiveFormat"]["backgroundColorStyle"].get("rgbColor")
        if csr_highlight is None:
            csr_highlight = self._parent.theme_colors[row[3]["effectiveFormat"]["backgroundColorStyle"]["themeColor"]]
        ...
        return SheetRecord(
            ...
            csr_data=_CSRDataBase(
                raw=[x.get("formattedValue", "") for x in row[3:]],
...
################################################################################
    def to_row_data(self, record: SheetRecord) -> Dict[str, List[Dict[str, Any]]]:
        ...
        csr_data = [self.empty_value()]
        csr: _CSRDataBase = record._csr_data
